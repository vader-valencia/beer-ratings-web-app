...
export const getItemsByCategoryId = (categoryId: number) => {
  const getItemsByCategoryIdUrl = `https://${ipAddress}:8000/${categoryId}/items`;

  const ratings = axios.get<DrinkItems>(getItemsByCategoryIdUrl).then(response => { return response.data });
...
export function getCategoryById(id: number) {
  const getCategoryByIdUrl = `https://${ipAddress}:8000/categories/${id}`;

  const categoryResponse = axios.get<Category>(getCategoryByIdUrl).then((response) => {
    return response.data;
  })
